...
class Element(Node):
    ...
    def _report_child(
        ...
        if isinstance(child, Text):
            return f"{msg}  Expecting child of type <{_type}>, " f'not text data "{child.astext()}".'
            ...
            for id in node["ids"]:
                ...
                if self.ids[id] is not node:
                    msg = self.reporter.error(
                        f'Duplicate ID: "{id}" used by ' f"{self.ids[id].starttag()} " f"and {node.starttag()}",
                        base_node=node,
                    )
...
ATTRIBUTE_VALIDATORS: dict[str, Callable[[str], Any]] = {
    ...
    "enumtype": create_keyword_validator("arabic", "loweralpha", "lowerroman", "upperalpha", "upperroman"),
    "format": str,  # CDATA (space separated format names)
    "frame": create_keyword_validator("top", "bottom", "topbot", "all", "sides", "none"),  # from CALS, ignored
