...
class Reporter:
    ...
    def system_message(self, level: int, message: str, *children, **kwargs: Any) -> nodes.system_message:
        """
    ...
    while line:
        ...
        if line[0] in "'\"":
            ...
            if len(line) > endquote_index + 1 and line[endquote_index + 1].strip():
                raise NameValueError(f'attribute "{attname}" end quote ' f"({line[0]}) not followed by whitespace")
...
def normalize_language_tag(tag: str) -> list[str]:
    ...
    taglist = [
        "-".join(base_tag + tags) for n in range(len(subtags), 0, -1) for tags in itertools.combinations(subtags, n)
    ]
    taglist += base_tag
    return taglist
...
class DependencyList:
    ...
    def __init__(
        self, output_file: Literal["-"] | StrPath | None = None, dependencies: Iterable[StrPath] = ()
    ) -> None:
        """
