...
def _check_result(result, details=[]):
    ...
    if details:
        ...
        msg = f"TeX to MathMl converter `{result.args[0]}` " f"exited with Errno {result.returncode}."
...
def blahtexml(math_code, as_block=False) -> str:
    ...
    if "<error>" in result.stdout:
        result.stderr = result.stdout[result.stdout.find("<message>") + 9 : result.stdout.find("</message>")]
    else:
        result.stdout = result.stdout[result.stdout.find("<markup>") + 9 : result.stdout.find("</markup>")]
    _check_result(result)
    return f'<math xmlns="http://www.w3.org/1998/Math/MathML"{mathml_args}>' f"\n{result.stdout}</math>"
...
def latexml(math_code, as_block=False):
    ...
    if result1.stderr:
        result1.stderr = "\n".join(line for line in result1.stderr.splitlines() if line.startswith(error_tags))
    ...
    if start != -1:
        ...
        result2.stdout = result2.stdout[start:stop].replace("ltx_equationgroup", "borderless align-center")
    else:
        result2.stdout = result2.stdout[result2.stdout.find("<math") : result2.stdout.find("</math>") + 7]
    # Search for error messages
    if result2.stdout:
        _msg_source = result2.stdout  # latexmlpost reports errors in output
    else:
        _msg_source = result2.stderr  # just in case
    result2.stderr = "\n".join(line for line in _msg_source.splitlines() if line.startswith(error_tags))
...
def pandoc(math_code, as_block=False):
    ...
    result = subprocess.run(args, input=wrap_math_code(math_code, as_block), capture_output=True, text=True)
...
def ttm(math_code, as_block=False):
    ...
    try:
        result = subprocess.run(args, input=math_code, capture_output=True, text=True, encoding="ISO-8859-1")
    ...
    result.stderr = "\n".join(line[5:] + "." for line in result.stderr.splitlines() if line.startswith("**** "))
    _check_result(result)
    return result.stdout
...
if __name__ == "__main__":
    example = r"\frac{\partial \sin^2(\alpha)}{\partial \vec r}" r"\varpi \mathbb{R} \, \text{Grüße}"
