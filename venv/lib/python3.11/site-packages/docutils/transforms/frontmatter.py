...
class DocInfo(Transform):
    ...
    def apply(self) -> None:
        ...
        if isinstance(candidate, nodes.field_list):
            biblioindex = document.first_child_not_matching_class((nodes.Titular, nodes.decoration, nodes.meta))
    ...
    def check_compound_biblio_field(self, field, name) -> bool:
        ...
        content = [f"<{e.tagname}>" for e in f_body.children if not isinstance(e, nodes.system_message)]
        if len(content) > 1:
    ...
    rcs_keyword_substitutions = [
        (
            re.compile(
                r"\$" r"Date: " r"(\d\d\d\d)[-/](\d\d)[-/](\d\d)[ T][\d:]+" r"[^$]* \$", re.IGNORECASE  # NoQA: ISC001
            ),
            r"\1-\2-\3",
        ),
