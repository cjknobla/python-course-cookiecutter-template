    ...
    def get_opening(self, width=r"\linewidth"):
        ...
        if not self.colwidths_auto:
            if self.borders == "standard" and not self.legacy_column_widths:
                opening.append(
                    r"\setlength{\DUtablewidth}" r"{\dimexpr%s-%i\arrayrulewidth\relax}%%" % (width, nr_of_cols + 1)
                )
            else:
...
class LaTeXTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document, babel_class=Babel) -> None:
        ...
        if settings.literal_block_env:
            (_none, self.literal_block_env, self.literal_block_options, _none) = re.split(
        ...
        # Document parts
        self.head_prefix = [f"\\documentclass[{self.documentoptions}]" f"{{{settings.documentclass}}}"]
        ...
        if self.hyperlink_color.lower() not in ("0", "off", "no", "false", ""):
            self.hyperref_options.append(
                "colorlinks=true," f"linkcolor={self.hyperlink_color}," f"urlcolor={self.hyperlink_color}"
            )
    ...
    def visit_docinfo_item(self, node, name=None) -> None:
        ...
        if name is not None:
            warnings.warn(
                'visit_docinfo_item(): argument "name" is obsolete' " and will be removed in Docutils 0.24",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def visit_entry(self, node):
        ...
        if "morerows" in node and "morecols" in node:
            raise NotImplementedError(
                "Cells that span multiple rows *and* " "columns currently not supported " "by the LaTeX writer"
            )
    ...
    def to_latex_length(self, length_str, node=None) -> str:
        ...
        if unit in ("em", "ex", "cm", "mm", "in", "pc", "px", "bp", "cc", "dd", "sp", "mu"):  # TeX unit == CSS unit
            return length_str
