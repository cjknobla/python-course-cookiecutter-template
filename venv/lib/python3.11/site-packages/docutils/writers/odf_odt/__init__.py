    ...
    settings_spec = (
        ...
        (
            (
                ...
                ["--stylesheet"],
                {"default": default_stylesheet_path, "dest": "stylesheet", "metavar": "<filename>"},
            ),
            (
...
class ODFTranslator(nodes.GenericNodeVisitor):
    ...
    def __init__(self, document) -> None:
        ...
        if self.settings.output_encoding == "unicode":
            self.document.reporter.severe("The ODT writer returns `bytes` " "that cannot be decoded to `str`")
    ...
    def visit_image(self, node) -> None:
        ...
        if uri_parts.scheme in ("", "file"):
            ...
            if not self.check_file_exists(source):
                self.document.reporter.warning(f'Cannot find image file "{source}".', base_node=node)
                return
        if source in self.image_dict:
            ...
            if uri_parts.scheme in ("", "file"):
                ...
                try:
                    ...
                    self.document.reporter.warning(f'Cannot open image URL "{source}". {err}', base_node=node)
                    return
    ...
    def get_image_width_height(self, node, attr):
        ...
        if attr in node.attributes:
            ...
            try:
                ...
                self.document.reporter.warning(
                    f'Invalid {attr} for image: "{node.attributes[attr]}".  ' f'Error: "{exp}".', base_node=node
                )
    ...
    def get_image_scale(self, node):
        if "scale" in node.attributes:
            ...
            if scale < 1:
                self.document.reporter.warning(f"scale out of range ({scale}), using 1.", base_node=node)
                scale = 1
            scale = scale * 0.01
        else:
            scale = 1.0
    ...
    def get_image_scaled_width_height(self, node, source):
        ...
        if width is None or height is None:
            if PIL is None:
                self.document.reporter.warning("image size not fully specified and PIL not installed", base_node=node)
    ...
    def visit_option_list(self, node) -> None:
        ...
        if not self.optiontablestyles_generated:
            ...
            el1 = SubElement(
                el, "style:table-column-properties", attrib={"style:column-width": "4.999cm"}, nsdict=SNSD
            )
            el = SubElement(
                self.automatic_styles,
                "style:style",
    ...
    def visit_reference(self, node) -> None:
        ...
        if self.settings.create_links:
            if "refuri" in node:
                ...
                self.document.reporter.warning('References must have "refuri" or "refid" attribute.', base_node=node)
        if (
    ...
    def get_table_style(self, node):
        ...
        if table_name is not None:
            ...
            if table_style is None:
                ...
                self.document.reporter.warning(
                    f'Can\'t find table style "{table_name}".  Using default.', base_node=node
                )
                ...
                if table_style is None:
                    ...
                    self.document.reporter.warning(
                        f'Can\'t find default table style "{table_name}".  ' "Using built-in default.", base_node=node
                    )
            ...
            if table_style is None:
                ...
                #   and use a built-in default style.
                self.document.reporter.warning(
                    'Can\'t find default table style "{table_name}".  ' "Using built-in default.", base_node=node
                )
                table_style = BUILTIN_DEFAULT_TABLE_STYLE
        return table_style
