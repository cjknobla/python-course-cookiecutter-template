...
from docutils.utils.math import latex2mathml, math2html, tex2mathml_extern, unichar2tex, wrap_math_code, MathError
    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                ["--math-output"],
                {"metavar": "<format [option(s)]>", "default": "MathML", "validator": frontend.validate_math_output},
            ),
            (
...
class HTMLTranslator(writers.DoctreeTranslator):
    ...
    def read_size_with_PIL(self, node) -> tuple[int, int] | None:
        ...
        if reading_problems:
            msg = ["Cannot scale image!", f'Could not get size from "{uri}":', *reading_problems]
    ...
    def prepare_svg(self, code: str, node: nodes.Element, atts: dict) -> str:
        ...
        svg_ns = {"": "http://www.w3.org/2000/svg", "xlink": "http://www.w3.org/1999/xlink"}  # SVG namespace
        ...
        try:
            ...
            self.messages.append(
                self.document.reporter.error(f'Cannot parse SVG image "{node["uri"]}":\n  {err}', base_node=node)
            )
            return code
    ...
    def visit_document(self, node) -> None:
        title = node.get("title") or os.path.basename(node.get("source") or "") or "untitled Docutils document"
    ...
    def depart_document(self, node) -> None:
        ...
        if self.settings.output_encoding and self.settings.output_encoding.lower() != "unicode":  # may be None
    ...
    def visit_field_list(self, node) -> None:
        ...
        for i, cls in enumerate(classes):
            if cls.startswith("field-indent-"):
                try:
                    indent_length = length_or_percentage_or_unitless(cls.removeprefix("field-indent-"), "px")
                except ValueError:
                    break
    ...
    def visit_footnote(self, node) -> None:
        ...
        self.body.append(self.starttag(node, "aside", classes=[node.tagname, label_style], role="doc-footnote"))
    ...
    def visit_image(self, node) -> None:
        ...
        if loading == "lazy":
            ...
            try:
                ...
                self.messages.append(
                    self.document.reporter.error(f'Cannot embed image "{uri}":\n  {err}', base_node=node)
                )
        ...
        if mimetype in self.videotypes:
            ...
            atts["type"] = mimetype
            element = self.starttag(node, "object", "", data=uri, **atts) + f"{alt}</object>{suffix}"
    ...
    def visit_math(self, node):
            ...
            if not self.math_header:
                if self.math_options:
                    ...
                    self.document.reporter.warning(
                        "No MathJax URL specified, using local fallback " "(see config.html).", base_node=node
                    )
        ...
        suffix = "\n" if is_block else ""
        if tag:
            self.body.append(self.starttag(node, tag, suffix=suffix, classes=self.math_tags[format][2]))
    ...
    def visit_term(self, node) -> None:
        if "details" in node.parent.parent["classes"]:
            ...
            self.body.append(
                self.starttag(node, "dt", suffix="", classes=node.parent["classes"], ids=node.parent["ids"])
            )
