...
class Include(Directive):
    ...
    def read_file(self, path: StrPath) -> str:
        ...
        if starttext:
            ...
            if after_index < 0:
                raise self.severe('Problem with "start-after" option of ' f'"{self.name}" directive:\nText not found.')
            text = text[after_index + len(starttext) :]
        if endtext:
            # skip content in text after *and incl.* a matching text
            before_index = text.find(endtext)
            if before_index < 0:
                raise self.severe('Problem with "end-before" option of ' f'"{self.name}" directive:\nText not found.')
    ...
    def as_literal_block(self, text: str) -> list[nodes.literal_block]:
        ...
        literal_block = nodes.literal_block("", source=source, classes=self.options.get("class", []))
    ...
    def insert_into_input_lines(self, text: str) -> None:
        ...
        if (source, self.clip_options) in include_log:
            ...
            raise self.warning(f'circular inclusion in "{self.name}"' f" directive:\n{inclusion_chain}")
