...
class NestedStateMachine(RSTStateMachine):
    ...
    def __init__(self, state_classes, initial_state, debug=False, parent_state_machine=None) -> None:
...
class RSTState(StateWS):
    ...
    def nested_parse(
        ...
        if not my_state_machine:
            my_state_machine = state_machine_class(
                debug=self.debug, parent_state_machine=self.state_machine, **state_machine_kwargs
            )
        # Check if we may use sections (with a caveat for custom nodes
        # that may be dummies to collect children):
        if node == self.state_machine.node and not isinstance(node, (nodes.document, nodes.section)):
    ...
    def nested_list_parse(
        ...
        my_state_machine.run(block, input_offset, memo=self.memo, node=node, match_titles=match_titles)
    ...
    def title_inconsistent(self, sourcetext, lineno):
        ...
        error = self.reporter.error("Title level inconsistent:", nodes.literal_block("", sourcetext), line=lineno)
        return error
...
class Inliner:
    ...
    def init_customizations(self, settings) -> None:
        ...
        self.patterns = Struct(
            ...
            substitution_ref=re.compile(self.non_whitespace_escape_before + r"(\|_{0,2})" + end_string_suffix),
...
class Body(RSTState):
    ...
    def enumerator(self, match, context, next_state):
        ...
        if ordinal != 1:
            ...
            msg = self.reporter.info(
                'Enumerated list start value not ordinal-1: "%s" (ordinal %s)' % (text, ordinal), base_node=enumlist
            )
            self.parent += msg
    ...
    def isolate_grid_table(self):
        ...
        for i in range(len(block)):  # check right edge
            if len(strip_combining_chars(block[i])) != width or block[i][-1] not in "+|":
            ...
            if match:
                if len(line.strip()) != toplen:
                    ...
                    messages = self.malformed_table(
                        lines[start : i + 1], "Bottom border or header rule does " "not match top border.", i - start
                    )
    ...
    def substitution_def(self, match):
        ...
        for node in substitution_node[:]:
            if not (isinstance(node, nodes.Inline) or isinstance(node, nodes.Text)):
                ...
                i += 1
        for node in substitution_node.findall(nodes.Element, include_self=False):
            if isinstance(node, nodes.problematic):
                ...
                msg.append(nodes.block_quote("", nodes.paragraph("", "", *substitution_node.children)))
                return [msg], blank_finish
            illegal = self.disallowed_inside_substitution_definitions(node)
            if illegal:
                msg = self.reporter.error(
                    f"{illegal} are not supported in " "a substitution definition.",
                    nodes.literal_block("", blocktext),
                    source=src,
                    line=srcline,
                )
    ...
    def parse_directive_arguments(self, directive, arg_block):
        ...
        if len(arguments) < required:
            ...
            if directive.final_argument_whitespace:
                ...
                raise MarkupError(
                    "maximum %s argument(s) allowed, %s supplied" % (required + optional, len(arguments))
                )
        return arguments
...
class SubstitutionDef(Body):
    ...
    patterns = {"embedded_directive": re.compile(r"(%s)::( +|$)" % Inliner.simplename), "text": r""}
        ...
        for i in range(len(text_nodes)):
            ...
            if isinstance(node, nodes.Text):
                ...
                if len(parts) == 1:
                    ...
                    node_list += [nodes.classifier(unescape(part, True), part) for part in parts[1:]]
            else:
                node_list[-1] += node
