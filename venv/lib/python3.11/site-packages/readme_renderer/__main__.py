...
def main(cli_args: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument("-p", "--package", help="Get README from package metadata", action="store_true")
    parser.add_argument(
        "-f", "--format", choices=["md", "rst", "txt"], help="README format (inferred from input file name or package)"
    )
    ...
    if args.package:
        ...
        if not content_format:
            ...
            if content_type == "text/x-rst":
                ...
                raise ValueError(f"invalid content type {content_type} for package " "`long_description`")
    else:
    ...
    if content_format == "md":
        ...
        raise ValueError(f"invalid README format: {content_format} (expected `md`, " "`rst`, or `txt`)")
    if rendered is None:
        sys.exit(1)
    if args.output == "-":
