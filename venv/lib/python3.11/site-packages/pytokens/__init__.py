...
class Token:
    ...
    def to_byte_slice(self, source: str) -> str:
        ...
        if self.type == TokenType.dedent and self.start_index == len(source) + 1 and self.end_index == len(source) + 1:
            return ""
...
class FStringState:
    ...
    def consume_rbrace(self) -> None:
        assert self.state == FStringState.in_fstring_expr or self.state == FStringState.in_fstring_expr_modifier
...
class TokenIterator:
    ...
    def pop_fstring_quote(self) -> None:
        ...
        self.fstring_prefix, self.fstring_quote = (
            (None, None) if len(self.fstring_prefix_quote_stack) == 0 else self.fstring_prefix_quote_stack.pop()
        )
    ...
    def decimal(self) -> Token:
        ...
        while self.is_in_bounds() and (
            ...
            or (self.source[self.current_index] == "_" and self.source[self.current_index - 1].isdigit())
        ):
            self.advance()
        ...
        if self.current_index + 1 < len(self.source) and (
            ...
            and (
                ...
                or (
                    ...
                    and (self.source[self.current_index + 1] == "+" or self.source[self.current_index + 1] == "-")
    ...
    def hexadecimal(self) -> Token:
        ...
        self.advance()
        while self.is_in_bounds() and (
            self.source[self.current_index] in string.hexdigits or self.source[self.current_index] == "_"
        ):
            self.advance()
        if self.is_in_bounds() and (self.source[self.current_index] == "e" or self.source[self.current_index] == "E"):
            self.advance()
