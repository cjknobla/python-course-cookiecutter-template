...
def detect_github(audience: str) -> str | None:
    ...
    try:
        ...
        raise AmbientCredentialError(
            f"GitHub: OIDC token request failed (code={resp.status_code}, " f"body={resp.content.decode()!r})"
        ) from http_error
...
def detect_gcp(audience: str) -> str | None:
    ...
    if service_account_name:
        ...
        try:
            ...
            raise AmbientCredentialError(
                f"GCP: access token request failed (code={resp.status_code}, " f"body={resp.content.decode()!r})"
            ) from http_error
        except requests.Timeout:
        ...
        try:
            ...
            raise AmbientCredentialError(
                f"GCP: OIDC token request failed (code={resp.status_code}, " f"body={resp.content.decode()!r})"
            ) from http_error
        except requests.Timeout:
...
def detect_buildkite(audience: str) -> str | None:
    ...
    if shutil.which("buildkite-agent") is None:
        raise AmbientCredentialError("Buildkite: could not find Buildkite agent in Buildkite environment")
    ...
    if process.returncode != 0:
        raise AmbientCredentialError(f"Buildkite: the Buildkite agent encountered an error: {process.stdout}")
...
def detect_circleci(audience: str) -> str | None:
    ...
    if process.returncode != 0:
        raise AmbientCredentialError(f"CircleCI: the `circleci` tool encountered an error: {process.stdout}")
