...
def _is_valid_standalone_fmt_comment(comment: ProtoComment, leaf: Leaf, is_fmt_off: bool, is_fmt_skip: bool) -> bool:
    ...
    for leaf in body_node.leaves():
        if leaf.type == token.SEMI:
            ...
            if body_node.type == syms.suite:
                ...
                simple_stmts = [child for child in body_node.children if child.type == syms.simple_stmt]
...
def _generate_ignored_nodes_from_fmt_skip(leaf: Leaf, comment: ProtoComment, mode: Mode) -> Iterator[LN]:
...
def _contains_fmt_directive(comment_line: str, directives: set[str] = FMT_OFF | FMT_ON | FMT_SKIP) -> bool:
    """
