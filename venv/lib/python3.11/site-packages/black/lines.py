        ...
        for leaf_id, comments in self.comments.items():
            for comment in comments:
                if is_type_comment(comment, mode=self.mode):
                    if comment_seen or (
                        not is_type_ignore_comment(comment, mode=self.mode) and leaf_id not in ignored_ids
                    ):
                        return True
...
def is_line_short_enough(line: Line, *, mode: Mode, line_str: str = "") -> bool:  # noqa: C901
    ...
    for i, leaf in enumerate(line.leaves):
        ...
        if is_multiline_string(leaf):
            if leaf.parent and (
                leaf.parent.type == syms.test or (leaf.parent.parent and leaf.parent.parent.type == syms.dictsetmaker)
            ):
