...
class CParser(PLYParser):
    ...
    def _build_function_definition(self, spec, decl, param_decls, body):
        ...
        declaration = self._build_declarations(spec=spec, decls=[dict(decl=decl, init=None)], typedef_namespace=True)[
            0
        ]
    ...
    def p_error(self, p):
        ...
        if p:
            self._parse_error(
                "before: %s" % p.value, self._coord(lineno=p.lineno, column=self.clex.find_tok_column(p))
            )
        else:
