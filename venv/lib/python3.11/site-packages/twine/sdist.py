...
class TarGzSDist(SDist):
    def read(self) -> bytes:
        with tarfile.open(self.filename, "r:gz") as sdist:
            ...
            if root in {".", "/", ""}:
                raise exceptions.InvalidDistribution(f"Too many top-level members in sdist archive: {self.filename}")
            # ...containing the package metadata in a ``PKG-INFO`` file.
            with suppress(KeyError):
                member = sdist.getmember(root.rstrip("/") + "/PKG-INFO")
                if not member.isfile():
                    raise exceptions.InvalidDistribution(f"PKG-INFO is not a regular file: {self.filename}")
