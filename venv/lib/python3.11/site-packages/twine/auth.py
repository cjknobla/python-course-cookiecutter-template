...
class TrustedPublishingAuthenticator(requests.auth.AuthBase):
    ...
    def __call__(self, request: "requests.models.PreparedRequest") -> "requests.models.PreparedRequest":
        ...
        if token is None:
            raise exceptions.TrustedPublishingFailure("Expected a trusted publishing token but got None")
...
class Resolver:
    ...
    def authenticator(self) -> "requests.auth.AuthBase":
        ...
        raise exceptions.InvalidConfiguration("could not determine credentials for configured repository")
    ...
    def _make_trusted_publishing_token(self) -> t.Optional[TrustedPublishingToken]:
        ...
        try:
            ...
            raise exceptions.TrustedPublishingFailure(
                "Unable to retrieve an OIDC token from the CI platform for " f"trusted publishing {e}"
            )

        if oidc_token is None:
            logger.warning("This environment is not supported for trusted publishing")
            if self._tp_token and int(time.time()) > cast(int, self._tp_token.get("expires", self._expires)):
        ...
        try:
            ...
            raise exceptions.TrustedPublishingFailure("The token-minting request returned invalid JSON")
        ...
        if not mint_token_resp.ok:
            ...
            raise exceptions.TrustedPublishingFailure(
                "The token request failed; the index server gave the following" f" reasons:\n\n{reasons}"
            )
