    ...
    if upload_settings.attestations:
        ...
        if not attestations:
            raise exceptions.InvalidDistribution(
                "Upload with attestations requested, but " f"{filename} has no associated attestations"
            )
...
def upload(upload_settings: settings.Settings, dists: List[str]) -> None:
    ...
    packages_to_upload = [
        _make_package(filename, signatures, attestations_by_dist[filename], upload_settings) for filename in uploads
    ]
    ...
    if signatures and not packages_to_upload:
        raise exceptions.InvalidDistribution(
            "Cannot upload signed files by themselves, must upload with a " "corresponding distribution file."
        )
