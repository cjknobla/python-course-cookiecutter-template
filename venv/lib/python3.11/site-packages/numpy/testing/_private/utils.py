    ...
    *,
    ...
    try:
        ...
        if not cond:
            ...
            msg = build_err_msg(
                [x, y], err_msg + reason, verbose=verbose, header=header, names=names, precision=precision
            )
            raise AssertionError(msg)
        ...
        if cond != True:
            ...
            remarks = [f"Mismatched elements: {n_mismatch} / {n_elements} " f"({percent_mismatch:.3g}%)"]
            ...
            with errstate(all="ignore"):
                ...
                with contextlib.suppress(TypeError):
                    ...
                    if getattr(error, "dtype", object_) == object_:
                        remarks.append("Max absolute difference among violations: " + str(max_abs_error))
                    else:
                        remarks.append("Max absolute difference among violations: " + array2string(max_abs_error))
                    ...
                    if getattr(error, "dtype", object_) == object_:
                        remarks.append("Max relative difference among violations: " + str(max_rel_error))
                    else:
                        remarks.append("Max relative difference among violations: " + array2string(max_rel_error))
            err_msg = str(err_msg)
            err_msg += "\n" + "\n".join(remarks)
            msg = build_err_msg([ox, oy], err_msg, verbose=verbose, header=header, names=names, precision=precision)
            raise AssertionError(msg)
    except ValueError:
        import traceback
        ...
        msg = build_err_msg([x, y], err_msg, verbose=verbose, header=header, names=names, precision=precision)
...
def assert_allclose(actual, desired, rtol=1e-7, atol=0, equal_nan=True, err_msg="", verbose=True, *, strict=False):
    """
    Raises an AssertionError if two objects are not equal up to desired
    tolerance.
...
def assert_warns(warning_class, *args, **kwargs):
    ...
    **kwargs : Kwargs
    ...
    if not args and not kwargs:
        ...
        if "match" in kwargs:
            raise RuntimeError("assert_warns does not use 'match' kwarg, " "use pytest.warns instead")
...
def run_threaded(func, max_workers=8, pass_count=False, pass_barrier=False, outer_iterations=1, prepare_args=None):
