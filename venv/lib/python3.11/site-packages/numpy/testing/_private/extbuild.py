...
def _c_compile(cfile, outputfilename, include_dirs, libraries, library_dirs):
    ...
    return build(cfile, outputfilename, compile_extra, link_extra, include_dirs, libraries, library_dirs)
...
def build(cfile, outputfilename, compile_extra, link_extra, include_dirs, libraries, library_dirs):
    ...
    if sys.platform == "win32":
        ...
        subprocess.check_call(
            ["meson", "setup", "--vsenv", "..", f"--native-file={os.fspath(native_file_name)}"], cwd=build_dir
        )
