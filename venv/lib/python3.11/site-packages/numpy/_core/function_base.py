...
def _linspace_dispatcher(start, stop, num=None, endpoint=None, retstep=None, dtype=None, axis=None, *, device=None):
    return (start, stop)
...
def logspace(start, stop, num=50, endpoint=True, base=10.0, dtype=None, axis=0):
    ...
    if not isinstance(base, (float, int)) and np.ndim(base):
        ...
        start, stop, base = (np.array(a, copy=None, subok=True, ndmin=ndmax) for a in (start, stop, base))
...
def _add_docstring(obj, doc, warn_on_python):
    if warn_on_python and not _needs_add_docstring(obj):
        warnings.warn(
            f"add_newdoc was used on a pure-python object {obj}. " "Prefer to attach it directly to the source.",
            UserWarning,
            stacklevel=3,
        )
    try:
