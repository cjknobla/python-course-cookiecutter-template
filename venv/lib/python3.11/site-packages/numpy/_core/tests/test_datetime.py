...
class TestDateTime:
    ...
    def test_timedelta_array_with_nats(self):
        ...
        x = np.array(
            [np.timedelta64("nat"), np.timedelta64("nat", "s"), np.timedelta64("nat", "ms"), np.timedelta64(123, "ms")]
        )
        for td in x[:3]:
            assert np.isnat(td)
    ...
    def test_datetime_as_string(self):
        ...
        assert_equal(
            np.datetime_as_string(np.datetime64("2032-07-18T12:23:00", "us"), unit="auto"), "2032-07-18T12:23"
        )
        # 'auto' doesn't split up hour and minute
        assert_equal(
            np.datetime_as_string(np.datetime64("2032-07-18T12:00:00", "us"), unit="auto"), "2032-07-18T12:00"
        )
    ...
    def test_datetime_arange(self):
        ...
        assert_equal(
            a, np.array(["2010-01-05", "2010-01-06", "2010-01-07", "2010-01-08", "2010-01-09"], dtype="M8[D]")
        )
    ...
    def test_datetime_busday_holidays_offset(self):
        ...
        assert_equal(
            np.busday_offset("2011-12-26", 0, roll="backward", holidays=holidays), np.datetime64("2011-12-23")
        )
        assert_equal(
            np.busday_offset(
                "2012-02-27",
