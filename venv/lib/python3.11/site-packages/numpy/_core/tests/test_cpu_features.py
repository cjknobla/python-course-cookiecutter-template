...
if __name__ == "__main__":
    ...
    @pytest.mark.skipif(
        not __cpu_dispatch__, reason=("NPY_*_CPU_FEATURES only parsed if " "`__cpu_dispatch__` is non-empty")
    )
    ...
    def test_impossible_feature_enable(self):
        ...
        if self.BASELINE_FEAT is not None:
            ...
            self.env["NPY_ENABLE_CPU_FEATURES"] = feats
            msg = (
                f"You cannot enable CPU features \\({bad_feature}\\), since " "they are not supported by your machine."
            )
            self._expect_error(msg, err_type)
...
class Test_ARM_Features(AbstractTest):
    ...
    def load_flags(self):
        ...
        is_rootfs_v8 = not re.match(r"^armv[0-9]+l$", machine) and (int("0" + next(iter(arch))) > 7 if arch else 0)
